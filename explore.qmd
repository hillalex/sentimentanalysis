...
df = pd.read_csv("data/scoredalltweets.csv")
df_preg = pd.read_csv("data/scoredpregnancytweets.csv")
```
...
attributable to the tweet being spam.
...
Let's try and remove tweets with links in them.
...
```{python}
tab = df[df.sentiment == 0].head().text
from IPython.display import Markdown
from tabulate import tabulate
Markdown(tabulate(list(map(lambda x:[x], tab))))
```
...
Three are about abortion and none are about the experience of being pregnant.
...
about the experience of pregnancy, and none seem like their positive sentiment score is very accurate!
